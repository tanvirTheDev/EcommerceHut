      ...
      <PopoverContent className="w-full p-0">
        <Command>
          <CommandInput
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                const selelctedCategory = categories.find((c) =>
                  c.title
                    ?.toLowerCase()
                    .includes(e.currentTarget.value.toLowerCase())
                );
                if (selelctedCategory?.slug?.current) {
                  setValue(selelctedCategory._id);
                  router.push(`/category/${selelctedCategory.slug.current}`);
                  setOpen(false);
                }
              }
            }}
